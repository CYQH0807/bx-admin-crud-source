...
export default {
  ...
  methods: {
    ...
    // getIsEdit(prop) {
    //   // 全局edit
    //   const confEdit = this.conf.isEdit
    //   // prop 属性的edit
    //   let propEdit = true
    //   const items = this.conf.items
    //   const propObject = items.find(item => item.prop === prop)
    //   if (propObject) {
    //     propEdit = propObject.isEdit === undefined ? true : propObject.isEdit
    //   } else {
    //     console.error(`没有找到${prop}的数据箱`)
    //     return true
    //   }
    //   if (!propEdit) {
    //     return false
    ...
    setItems(prop, data) {
      const propIndex = this.conf.items.findIndex(item => item.prop === prop)
      if (propIndex > -1) {
        for (const key in data) {
          this.$set(this.conf.items[propIndex], key, data[key])
        }
      }
    },
    ...
    // 设置表单项数据
    setData(path, value) {
      this._set({ path, isMerge: true }, value)
    },
