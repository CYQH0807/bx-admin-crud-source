  ...
  methods: {
          ...
          const deep = (item) => {
            ...
            if (!item.type || item.type === 'expand') {
              params.scopedSlots = {
                ...
                header: (scope) => {
                  const slot = this.$scopedSlots[`header-${item.prop}`]
        ...
        const list = buttons
          .map((e) => {
            switch (e) {
              ...
              case 'check':
                return {
                  label: selection.find((e) => e.id == row.id)
                    ? '取消选择'
                    : '选择',
    ...
    calcMaxHeight() {
      ...
      return this.$nextTick(() => {
        ...
        if (el) {
          const rows = el.querySelectorAll('.cl-crud .el-row')
          ...
          const h1 = Number(String(height).replace('px', ''))
